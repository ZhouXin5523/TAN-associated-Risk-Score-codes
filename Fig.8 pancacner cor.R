...
up <- read.table("score p.txt",sep = "\t",check.names = F,header = T,row.names=1)  
dn <- read.table("score cor.txt",sep = "\t",check.names = F,header = T,row.names=1)     
dn=t(dn)
up=t(up)

colVector=c("#AB221F","#3878C1","#FFFADD","#00FF66")
...
rbPal1 <- colorRampPalette(colors = c(colVector[4],"white",colVector[1]))
...
rbPal2 <- colorRampPalette(colors = c(colVector[3],"white",colVector[2]))
...
pdf("score heatmap.pdf",width = 8,height = 2)
layout(mat=matrix(c(1,0,1,2,1,0,1,3,1,0),5,2,byrow=T),widths=c(length(cancer.level),2))


par(bty="n", mgp = c(2,0.5,0), mar = c(5.1, 20, 3, 3),tcl=-.25,xpd = T)   
...
par(mar=c(0,0,0,2),xpd = T,cex.axis=1.2)
